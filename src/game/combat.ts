...
export class Combat {
  ...
  constructor(ui: Game_UI) {
    ...
    this.player = {
      ...
      y: this.world.endPosition.y,
      symbol: "☻", // Temporary player ascii
      health: 100,
      facing: "right",
      velocityY: 0,
      isJumping: false,
  ...
  async start(): Promise<void> {
      this.isRunning = true;
      await this.ui.add_log("Combat was running", 0);
...
class WorldGenerator {
  static generateLevel(level: number): WorldLevel {
    ...
    //------- ~ Gnerate terrain based on level theme ~ -------
    switch (level) {
      case 1:
  ...
  /*=======================================================*
 |                World Generator Utils                  |
 *=======================================================*/

  //@ts-expect-error
  private static generateEnemies(level: number, tiles: unknown): Enemy[] {}

  //@ts-expect-error
  private static generateItems(level: number): Item[] {}
...
//@ts-expect-error
export const combat = new Combat(Game_UI);